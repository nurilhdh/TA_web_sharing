<!doctype html>
<html lang="en">
<head>
    <!-- Required meta tags -->
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equive="X-UA-Compatible" content="ie=edge">
    <title>Accordian</title>
    <link rel="stylesheet" href="css/style_faq.css">
    <script src="js/main_faq.js" defer></script>
    
</head>
<body>
    <h1>Frequently Asked Questions (FAQ)</h1>
   
<div class="accordion">
  <div class="accordion-item">
    <div class="accordion-item-header">
      Apa yang dimaksud Secure File Sharing?
    </div>
    <div class="accordion-item-body">
      <div class="accordion-item-body-content">
      File Sharing atau berbagi file adalah kegiatan berbagi atau menawarkan akses ke informasi atau sumber daya digital 
      dengan menerapkan sistem keamanan agar pengguna lebih nyaman. Website ini dapat digunakan untuk file dengan maksimal ukuran 10 mb dan hanya dari format .pdf dan .doc
      </div>
    </div>
  </div>
  <div class="accordion-item">
    <div class="accordion-item-header">
      Bagaimana penerapan sistem keamanan pada dokumen?
    </div>
    <div class="accordion-item-body">
      <div class="accordion-item-body-content">
       Sistem keamanan menggunakan metode algoritma kriptografi Advanced Encryption Standard (AES). Algoritma AES ini untuk melakukan 
enkripsi dan dekripsi file, sehingga file asli hanya dapat dibaca oleh penerima jika menerima kunci dari pengirim. 
      </div>
    </div>
  </div>
  <div class="accordion-item">
    <div class="accordion-item-header">
      Bagaimana cara menerapkan enkripsi file dari pengirim?
    </div>
    <div class="accordion-item-body">
      <div class="accordion-item-body-content">
        <ul style="padding-left: 1rem;">
          <li>Login akun user. Jika belum, Anda bisa melakukan registrasi terlebih dahulu.</li>
          <li>Bagi Pengirim File, maka Anda bisa klik button Upload File Here untuk menuju halaman upload</li>
          <li>Selanjutnya Anda dapat upload file dengan cara klik icon file untuk memilih file yang akan diupload</li>
          <li>Masukkan key atau kunci yang digunakan</li>
          <li>Lalu, Klik button upload</li>
          <li>Anda juga bisa mendapatkan link untuk dibagikan ke penerima melalui button Get Sharable Link</li>
          <li>Jika sudah selesai, Anda dapat klik Logout untuk kembali ke halaman utama</li>
          <!-- <li>CSS Preprocessing</li> -->
         
        </ul>
      </div>
    </div>
  </div>
  <div class="accordion-item">
    <div class="accordion-item-header">
    Bagaimana cara menerapkan dekripsi file untuk penerima?
    </div>
    <div class="accordion-item-body">
      <div class="accordion-item-body-content">
        <ul style="padding-left: 1rem;">
          <li>Login akun user. Jika belum, Anda bisa melakukan registrasi terlebih dahulu.</li>
          <li>Bagi Penerima File, maka Anda bisa klik button Download File Here untuk menuju halaman download</li>
          <li>Selanjutnya Anda dapat download file dengan cara memasukkan key di kotak Key File</li>
          <li>Lalu, Anda dapat memberikan link yang didapatkan dari Get Sharable Link untuk diberikan di kotak hidden</li>
          <li>Lalu, Klik button Get File</li>
          <li>Jika sudah selesai, Anda dapat klik Logout untuk kembali ke halaman utama</li>
          <!-- <li>CSS Preprocessing</li> -->
         
        </ul>
      </div>
    </div>
  </div>
    <!-- </div>
    <div class="accordion-item-body">
      <div class="accordion-item-body-content">
        HTTP, aka HyperText Transfer Protocol, is the underlying protocol used by the World Wide Web and this protocol defines how messages are formatted and transmitted, and what actions Web servers and browsers should take in response to various commands.
      </div>
    </div> -->
  </div>
  <div class="accordion-item">
    
    </div>
  </div>
</div>
</body>